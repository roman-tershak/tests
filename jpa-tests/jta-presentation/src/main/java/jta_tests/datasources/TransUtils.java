...
public class TransUtils {
	...
	public static boolean pauseOnPrepare;
	public static boolean pauseOnCommit;
	...
	private static final class GenericInvocationHandler implements InvocationHandler {
		...
		private void delayCall(Method method) throws Exception {
			if (pauseOnPrepare || pauseOnCommit) {
				if (xaDataSource != null && (xaDataSource instanceof PGXADataSource ||
						xaDataSource instanceof OracleXADataSource)) {
					if (pauseOnPrepare && method.getName().equals("prepare")) {

						System.out.println("\n\n\n\nIn prepare\n\n\n\n");
						Thread.sleep(15000);

					} else if (pauseOnCommit && method.getName().equals("commit")) {
